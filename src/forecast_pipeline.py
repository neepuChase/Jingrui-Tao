...
def _run_single_model(cleaned_df: pd.DataFrame, imf_df: pd.DataFrame, config: ForecastConfig, outputs_dir: Path, model_type: str) -> tuple[pd.DataFrame, dict[str, list[float]], np.ndarray, np.ndarray, pd.Series]:
    ...
    for i, col in enumerate(imf_columns):
        y_true_i, y_pred_i, loss_history = _train_single_imf(imf_values[:, i], config, torch.device("cpu"), model_type=model_type)
...
def run_imf_forecast(
    ...
    for m in selected_models:
        forecast_df, losses, y_true, y_pred, ts = _run_single_model(cleaned_df, imf_df, cfg, outputs_dir, model_type=m)
