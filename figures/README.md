...
### `load_weather_correlation.svg`
**英文名称：Load Correlation with Temperature, Humidity, and Rainfall**

中文说明：负荷与气象因素相关性图。该图会提取数据中的温度、湿度和降雨量字段，并分别计算它们与负荷的皮尔逊相关系数，再以柱状图方式展示。柱子越接近 `1` 或 `-1`，说明该气象因素与负荷的线性相关性越强；正值表示同向变化，负值表示反向变化。仓库内示例文件使用 SVG 文本格式保存，避免提交二进制图片时出现平台限制。
